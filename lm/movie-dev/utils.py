...
def get_train_test_sents(corpus, split1=0.7, split2=0.15, categories='pos', shuffle=True):
    ...
    train_frac = split1
    split_idx1 = int(train_frac * len(sentences))
    split_idx2 = int((train_frac + split2) * len(sentences))
    train_sentences = sentences[:split_idx1]
    test_sentences = sentences[split_idx1:split_idx2]
    dev_sentences = sentences[split_idx2:]
    ...
    fmt = (len(dev_sentences), sum(map(len, dev_sentences)))
...
##
# Use this function
def load_corpus(name, split1=0.7, split2=0.15, V=10000, categories='pos', shuffle=0):
    """Load a named corpus and split train/test along sentences."""
    corpus = get_corpus(name)
    vocab = build_vocab(corpus, V)
    train_sentences, test_sentences, dev_sents = get_train_test_sents(corpus, split1, split2, categories, shuffle)
    train_ids = preprocess_sentences(train_sentences, vocab)
    test_ids = preprocess_sentences(test_sentences, vocab)
    return vocab, train_ids, test_ids, dev_sents
